...
$$
\text{Abundance}=\sum_{i=1}^{n}\left(k\times\frac{Ni_{\text{mappedreads}}\times{L_{\text{reads}}/L_{\text{ARGreferencesequence}}}}{N_{\text{cell}}}\right)
$$
...
For the data visualization by ARG subtype, we only considered relatively abundant ARG types (overall abundance > 0.009 cpc). Subtypes were also grouped arbitrarily prior visualization, with groupings primarily based on [CARD](https://card.mcmaster.ca/) gene families. The data was visualized using a stacked bar chart. More details on these groupings can be found in the R script below.
